...
def _parse_company_node(state: ResearchState) -> ResearchState:
    ...
    for keyword, mapped in sorted(_KEYWORD_SECTORS.items(), key=lambda kv: len(kv[0]), reverse=True):
...
def _web_research_node(state: ResearchState) -> ResearchState:
    ...
    web_facts: dict[str, Any] = {
        ...
        "last_round_amount_raised": llm_facts.get("last_round_amount_raised"),
        "last_post_money_valuation": (
            best.amount_usd if best and best.evidence_type in (
                "post_money_fresh", "post_money_stale"
            ) else llm_facts.get("last_post_money_valuation")
        ),
        ...
        with DDGS() as ddgs:
            for q_template in _SEARCH_QUERIES:
                ...
                for r in results:
                    ...
                    body = str(r.get("body") or r.get("snippet") or r.get("description") or "").strip()
...
def _merge_llm_into_package(
    ...
    if rev and isinstance(rev, (int, float)) and rev > 0:
        if rev not in pkg.revenue_signals:
...
def _assemble_node(state: ResearchState) -> ResearchState:
    ...
    if requested_methodology:
        chosen_methodology = requested_methodology
    else:
        chosen_methodology = pkg.recommended_methodology()
    ...
    if chosen_methodology == "direct_valuation":
        ...
        if assembled and missing_fields:
            # Fall back to next best
            fallback_method = (
                "last_round_market_adjusted"
                if pkg.best_post_money and pkg.best_round_date
                else "comparable_companies"
            )
            logger.info("direct_valuation incomplete, falling back to %s", fallback_method)
            chosen_methodology = fallback_method
            assembled = None
    ...
    # If still nothing, try all methods and pick the least-incomplete one
    if assembled is None:
        attempts = []
        for method in [
            "direct_valuation", "last_round_market_adjusted",
